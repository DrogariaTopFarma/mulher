...
document.addEventListener('DOMContentLoaded', function () {
  ...
  const categoryIcons  = { rua: '🌳', estabelecimento: '🏪', onibus: '🚌', estacao: '🚇' };
  ...
  function cardTemplate(loc, index) {
    ...
    const photoHtml = loc.photo
      ...
      : '<div class="card-photo card-photo-placeholder" aria-hidden="true">' + categoryIcons[loc.category] + '</div>';
    ...
    return (
      '<article class="card" style="animation-delay:' + (index * 0.06) + 's">' +
        ...
        '<div class="card-body">' +
          ...
          '<p class="card-neighborhood">📍 ' + escapeHtml(loc.neighborhood) + '</p>' +
          ...
          '<button type="button" class="btn-save' + (isSaved ? ' saved' : '') + '" data-id="' + loc.id + '">' +
            (isSaved ? '💗 Salvo na Rota' : '🤍 Salvar na Minha Rota') +
          '</button>' +
        '</div>' +
      '</article>'
    );
  ...
  navCadastrar.addEventListener('click', function () {
    setActiveBottomNav('navCadastrar');
    document.getElementById('cadastroSection').scrollIntoView({ behavior: 'smooth' });
  });
